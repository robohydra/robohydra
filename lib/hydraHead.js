...
HydraHead.prototype.canDispatch = function(path) {
    ...
    var headPath = this.path.replace(new RegExp('/$'), '');
    if (this.documentRoot || this.proxyTo) {
        return reqPath.match(new RegExp(headPath));
    } else {
        return reqPath === headPath;
    }
};
...
HydraHeadFilesystem.prototype._mandatoryProperties = ['path', 'documentRoot'];
...
HydraHeadProxy.prototype._mandatoryProperties = ['path', 'proxyTo'];
