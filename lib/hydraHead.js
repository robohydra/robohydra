...
function serveStaticFile(filePath, res, cb, opts) {
    ...
    fsObject.readFile(filePath, function(err, data) {
        if (err) {
            ...
            res.contentType(mimeObject.lookup(filePath));
            res.send(data);
        }
        cb();
    });
...
HydraHeadFilesystem.prototype._handle = function(req, res, cb) {
    ...
    var filePath = (this.documentRoot + '/' + req.url.replace(pathRe, '')).
        replace(new RegExp('//+'), '/');
    serveStaticFile(filePath, res, cb, {fs: this.fs, mime: this.mime});
};
...
var HydraHeadProxy = function(props) {
    ...
    this.proxyToPath = proxyUrl.pathname.replace(new RegExp('/$'), '');
};
for (p in HydraHead.prototype) {
