...
RoboHydra.prototype.registerPluginObject = function(name, rawPlugin, config) {
    var plugin = this._inflatePlugin(rawPlugin, config);
    plugin.name = name;
...
RoboHydra.prototype._inflatePlugin = function(pluginInfo, config) {
    // var pluginInfo = this._requirePluginModule(name, opts);
    var pluginConfig = {robohydra: this, path: pluginInfo.path};
    for (var p in config) { pluginConfig[p] = config[p]; }
    var modulesObject = this.getModulesObject(pluginConfig);
    ...
    this._jsFilesIn(testDir).forEach(function(testFile) {
        ...
        var testDefinition = testModule.getBodyParts(pluginConfig,
                                                     modulesObject);
