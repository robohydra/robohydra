...
(function () {
    ...
    /**
     * Current scenario/test results. Its keys are plugin names and
    ...
    RoboHydra.prototype._emptyPluginStructure = function(plugin) {
        var pluginToRegister = {
            ...
            webSocketHeads: plugin.webSocketHeads || []
        };
        // Deprecated, only kept for compatibility for a while
        if (plugin.tests) {
    ...
    RoboHydra.prototype.webSocketHandle = function(req, socket) {
        for (var i = 0, len = this._plugins.length; i < len; i++) {
            var plugin = this._plugins[i].plugin;
            for (var j = 0, len2 = plugin.webSocketHeads.length; j < len2; j++) {
                if (plugin.webSocketHeads[j].canHandle(req)) {
                    plugin.webSocketHeads[j].handle(req, socket);
                    return;
                }
            }
        }
    ...
    RoboHydra.prototype._inflatePlugin = function(pluginInfo) {
        ...
        for (var prop in inflatedPlugin) {
            if (['heads', 'tests', 'scenarios', 'webSocketHeads'].indexOf(prop) === -1) {
        ...
        inflatedPlugin.webSocketHeads = inflatedPlugin.webSocketHeads;
