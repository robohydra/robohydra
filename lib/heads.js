...
RoboHydraHead.prototype.canHandle = function(path) {
    ...
    // used. The former is currently a string with a regexp matching
    // supported paths (assuming ^ and $ and the beginning/end), while
    // the latter is a base path, and all paths inside that are
    // supported (used for filesystem serving and proxying)
    var reqPath = this.normalizePath(this.stripGetParams(path));
    if (this.mountPath) {
        var mountPath = this.normalizePath(this.mountPath);
        if (mountPath === '/') {
            return true;
        }
        var headPathRegExpEscaped = this._escapeRegExp(mountPath);
        if (! reqPath.match(new RegExp("^" + headPathRegExpEscaped))) {
            return false;
        }
        return reqPath.length === mountPath.length ||
            reqPath[mountPath.length] === '/';
    } else {
        return reqPath.match(new RegExp("^" + this.path + "$"));
    }
};
