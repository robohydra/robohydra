...
function propsForClass(classObject, object, moreProps) {
    var parentProps = {};
    classObject.mandatoryProperties.forEach(function(prop) {
        parentProps[prop] = object[prop];
    });
    classObject.optionalProperties.forEach(function(prop) {
        parentProps[prop] = object[prop];
    });
    for (var prop in moreProps) {
        parentProps[prop] = moreProps[prop];
    }
...
function roboHydraHeadType(settings) {
    ...
    var newConstructorFunction = function(props) {
        ...
        var parentPropBuilder = settings.parentPropBuilder ||
                function() { return {}; };
        var parentProps = propsForClass(parentClass,
                                        this,
                                        parentPropBuilder.call(this));
    ...
    newConstructorFunction.optionalProperties =
        (settings.optionalProperties || []).concat('name', 'detached');
...
var RoboHydraHeadProxy = roboHydraHeadType({
    ...
    optionalProperties: [{name: 'mountPath', defaultValue: '/'},
                         ...
                         {name: 'setHostHeader', defaultValue: false}],
