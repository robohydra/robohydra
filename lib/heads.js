...
function proxiedUrl(mountPath, reqUrl, proxyTo) {
    var requestUrlExtra = relativePath(mountPath, reqUrl),
        ...
        // for '/'
        urlPath = (proxyUrl.pathname || '/').replace(new RegExp('/$'), '') +
            requestUrlExtra;

    return proxyUrl.protocol + '//' + proxyUrl.host + urlPath;
}
...
var RoboHydraHeadFilesystem = robohydraHeadType({
    ...
    optionalProperties: [{name: 'mountPath', defaultValue: '/'},
                         'indexFiles',
                         {name: 'passThrough', defaultValue: false},
        ...
        return {
            ...
            handler: function(req, res, next) {
                ...
                var fakeRes = new Response().on('end', function(evt) {
                    if (self.passThrough && evt.response.statusCode === 404) {
                        next(req, res);
                    } else {
                ...
                serveStaticFile(filePath, fakeRes, {
                    fs: this.fs,
                    mime: this.mime,
                    headers: req.headers,
...
var RoboHydraHeadReplayer = robohydraHeadType({
    name: 'replayer',
    defaultPropertyObject: {traffic: {}},

    mandatoryProperties: ['traffic'],
    optionalProperties: [{name: 'mountPath', defaultValue: '/'}],

    init: function() {
        if (typeof this.traffic === 'string') {
            try {
                this.traffic = JSON.parse(this.traffic);
            } catch (e) {
                if (e.name === 'SyntaxError') {
                    throw new InvalidRoboHydraHeadException(
                        "Property 'traffic' must be a valid JSON string if " +
                            "it's a string"
                    );
                }
            }
        ...
        if (typeof this.traffic !== 'object') {
            throw new InvalidRoboHydraHeadException("Property 'traffic' must be an object or a string");
        }

        this.currentTrafficData = this.traffic;
        this.indexForUrl = {};
    },
    ...
    parentPropertyBuilder: function() {
        return {
            ...
            handler: function(req, res) {
                var urlResponses = this.currentTrafficData[req.url];
                if (urlResponses) {
                    var index = this.indexForUrl[req.url] || 0;
                    var currentResponse =
                            this.currentTrafficData[req.url][index];
                    if (currentResponse) {
                        this.indexForUrl[req.url] =
                            ((index + 1) %
                             this.currentTrafficData[req.url].length);
                        res.statusCode = currentResponse.statusCode || 200;
                        res.headers    = currentResponse.headers || {};
                        res.send(Buffer.from(currentResponse.body, 'base64'));
                        return;
                    }
                }
                res.statusCode = 404;
        ...
        return {
            handler: function(req, socket) {
                ...
                var constructor = this.webSocketConstructor,
                    ws = new constructor(proxyUrl, null, {headers: headers}),
                    connected = false;
                ...
                socket.on('message', function(message) {
                    ...
                    if (processedMessage !== false) {
                        if (connected) {
                            ...
                            console.error('Cannot forward message to ' + proxyUrl + ', not connected');
                        }
                    }
                });
...
exports.RoboHydraHeadReplayer = RoboHydraHeadReplayer;
