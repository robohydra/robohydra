...
Hydra.prototype._registerPluginObject = function(plugin) {
    ...
    var pluginInternalStructure = {headMap: {}, plugin: plugin};
...
Hydra.prototype.headForPath = function(url) {
    for (var i = 0, len = this._plugins.length; i < len; i++) {
        ...
        for (var j = 0, len2 = plugin.heads.length; j < len2; j++) {
            if (plugin.heads[j].canHandle(url)) {
...
Hydra.prototype.handle = function(req, res, cb) {
    var r = this.headForPath(req.url);
    if (typeof(r) !== 'undefined') {
        res.statusCode = 200;
        r.head.handle(req, res, cb);
    } else {
        res.statusCode = 404;
        res.send("Not Found");
        cb();
